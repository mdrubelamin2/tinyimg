...
import { ROOT, parseJSONC, SSIM_THRESHOLD_DEFAULT, PSNR_THRESHOLD_DEFAULT, QUALITY_THRESHOLDS_PATH } from './common.mjs';
...
function loadQualityThresholds(originalsDir) {
  ...
  try {
    ...
    for (const [k, v] of Object.entries(data)) {
      ...
      if (v && typeof v === 'object' && (v.ssimMin != null || v.psnrMin != null || v.formats)) out[k] = v;
    }
    return out;
  } catch {
    return {};
...
async function loadAndAlign(originalPath, optimizedPath) {
  ...
  if (opt.width !== orig.width || opt.height !== orig.height) {
    ...
    const resized = await sharp(buf, { raw: { width: opt.width, height: opt.height, channels: 4 } })
...
async function main() {
  ...
  console.log(`Comparing ${pairs.length} raster pair(s): SSIM >= ${SSIM_THRESHOLD_DEFAULT}, PSNR >= ${PSNR_THRESHOLD_DEFAULT} dB (adaptive: palette vs truecolor, transparent+webp)...`);
  const failures = [];
  ...
  if (failures.length > 0) {
    ...
    console.error('Inspect the reported image(s); adjust worker constants or quality-thresholds.jsonc.');
