...
export function classifyContent(imageData: ImageData): ContentPreset {
  try {
    ...
    colorBitVec.fill(0)
    const step =
      totalPx > MAX_PIXELS_FOR_CLASSIFY ? Math.ceil(totalPx / MAX_PIXELS_FOR_CLASSIFY) : 1
    let uniqueColors = 0
    ...
    if (uniqueColors < GRAPHIC_COLOR_THRESHOLD && entropy < GRAPHIC_ENTROPY_THRESHOLD)
      return 'graphic'
    return 'photo'
  } catch {
    return 'photo'
...
export function isSmallAndTransparent(
  width: number,
  height: number,
