...
async function decodeSvgBlobToBitmap(
  ...
  const tryBitmap = async (b: Blob, opts?: ImageBitmapOptions): Promise<ImageBitmap | null> => {
    try {
      return opts == null ? await createImageBitmap(b) : await createImageBitmap(b, opts)
    } catch (error) {
  ...
  let bitmap =
    (await tryBitmap(blob, {
      resizeHeight: physH,
      resizeQuality: 'high',
      resizeWidth: physW,
    })) ?? (await tryBitmap(blob, { resizeHeight: physH, resizeWidth: physW }))
  ...
  if (!bitmap) {
    try {
      ...
      bitmap =
        (await tryBitmap(b2, { resizeHeight: physH, resizeWidth: physW })) ?? (await tryBitmap(b2))
    } catch (error) {
      errors.push(error instanceof Error ? error.message : String(error))
    }
  }
...
function injectSvgDimensionsStringFallback(svgText: string, width: number, height: number): string {
  ...
  attrs = attrs.replaceAll(/\bwidth\s*=\s*["'][^"']*["']\s*/gi, '')
  attrs = attrs.replaceAll(/\bheight\s*=\s*["'][^"']*["']\s*/gi, '')
