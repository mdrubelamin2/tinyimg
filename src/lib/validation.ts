...
export async function validateFile(
  ...
  if (!isValidImageExtension(ext)) {
    return { error: ERR_INVALID_FILE, ok: false }
  }
  if (file.size > MAX_FILE_SIZE_BYTES) {
    return { error: ERR_FILE_EXCEEDS_LIMIT, ok: false }
  }
...
function checkMagicBytesFromBuffer(b: Uint8Array, ext: string): boolean {
  ...
  switch (ext) {
    case 'avif': {
      return b.length >= 12 && eq(4, [0x66, 0x74, 0x79, 0x70])
    } // ftyp
    case 'heic':
    case 'heif': {
      return (
        b.length >= 12 && eq(4, [0x66, 0x74, 0x79, 0x70]) // ftyp
      )
    }
    case 'jpeg':
    case 'jpg': {
...
export { DEFAULT_MIME, getMimeType, mimeByFormat } from '@/constants'
