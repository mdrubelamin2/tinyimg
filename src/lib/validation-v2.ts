 ...
 * Includes HEIC browser support detection.
 */
...
function checkMagicBytesFromBuffer(b: Uint8Array, ext: string): boolean {
  ...
  switch (ext) {
    ...
    case 'heic':
    case 'heif':
      return b.length >= 12 && eq(4, [0x66, 0x74, 0x79, 0x70]); // ftyp (shared with AVIF)
    default:
      return false;
  }
}
 ...
 * Safari 17+ supports HEIC natively via createImageBitmap.
 * Chrome and Firefox do not (requires libheif-js WASM fallback).
 */
export function isHeicDecodeLikelySupported(): boolean {
  if (typeof navigator === 'undefined') return false;
  const ua = navigator.userAgent;
  // Safari on macOS/iOS has native HEIC support
  const isSafari = /Safari/.test(ua) && !/Chrome|Chromium|Edg/.test(ua);
  if (isSafari) return true;
  // TODO: Once we add libheif-js WASM decoder, return true unconditionally
  return false;
}
