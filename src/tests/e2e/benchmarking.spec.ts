...
const sizeOverrides: Record<string, Record<string, number>> = fs.existsSync(sizeOverridesPath)
  ? parseJSONC(fs.readFileSync(sizeOverridesPath, 'utf-8'))
  : {};

test.describe('Industrial Benchmarking', () => {
  const expectedPath = path.join(testImagesDirRoot, 'expected.jsonc');
  const expectedData = parseJSONC(fs.readFileSync(expectedPath, 'utf-8'));
  const testImagesDir = testImagesDirRoot;

  test.beforeEach(async ({ page }) => {
    test.slow();
    fs.mkdirSync(TEST_OUTPUT_DIR, { recursive: true });
    await page.goto('/', { waitUntil: 'load', timeout: E2E_PAGE_LOAD_TIMEOUT_MS });
  ...
  for (const [filename, expected] of Object.entries(expectedData)) {
    test(`should optimize ${filename} within expected limits`, async ({ page }) => {
      ...
      const row = page.locator('tr').filter({ has: page.locator('td').first().locator(`text="${filename}"`).first() }).first();
      await expect(row).toBeVisible({ timeout: E2E_PAGE_LOAD_TIMEOUT_MS });
      
      const expectedFormats = typeof expected === 'string' ? ['svg'] : Object.keys(expected as Record<string, string>);
      
      for (const format of expectedFormats) {
        const resultCell = row.locator('td').nth(2);
        const formatChip = resultCell.locator('div.rounded-xl').filter({ hasText: new RegExp(format, 'i') }).first();
        ...
        if (filename.endsWith('.svg') && !(await chipToUse.isVisible({ timeout: E2E_CHIP_VISIBLE_SHORT_MS }))) {
          continue;
        }

        await expect(chipToUse).toBeVisible({ timeout: E2E_BENCHMARK_CHIP_TIMEOUT_MS });
        await expect(chipToUse).toContainText('KB', { timeout: E2E_BENCHMARK_CHIP_TIMEOUT_MS });

        const sizeText = await chipToUse.locator('span.text-xs').innerText();
        const actualSize = parseFloat(sizeText.replace(' KB', '')) * BYTES_PER_KB;
        
        const expectedSizeStr = typeof expected === 'string' ? expected : (expected as Record<string, string>)[format];
